// Radiuino4App_Sensor : firmware para os nós Sensores da rede
...
{ 
  ...
  //+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=
  //                    ENDEREÇO DO NÓ SENSOR
  //+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=
  // Endereço ou ID do nó sensor, que pode ser escolhido de 0 a 255. Só pode existir um endereço por nó sensor
  my_addr = 1;               /* Endereço */
...
/**
 * Realiza uma operação Atômica para verificar se deve entrar no modo Sleep ou se deve sair do modo Sleep
 */
void verifySleepEntering ( void ) {
  ATOMIC_BLOCK(ATOMIC_FORCEON)
  {
    if (Mac.time_to_sleep > 0) {
      ...
      /* Aguarda o fim da tranmissão do ACK SLEEP */
